class TaskModel {
  ...
  double get completionPercentage => participantsNeeded == 0
      ? 0
      : (participantsCompleted / participantsNeeded) * 100;

  factory TaskModel.fromJson(Map<String, dynamic> j) => TaskModel(
      id: j['id'] ?? '',
      title: j['title'] ?? '',
      description: j['description'] ?? '',
      category: j['category'] ?? 'Growth',
      ...
      reward: j['reward'] ?? 0,
      verificationReward: j['verification_reward'] ?? 3,
      participantsNeeded: j['participants_needed'] ?? 0,
      participantsCompleted: j['participants_completed'] ?? 0,
      creatorId: j['creator_id'] ?? '',
      creatorName: j['creator_name'] ?? '',
      creatorImage: j['creator_image'],
